...
- **Platform variants are unbundled.** Packages carrying `os`/`cpu` resolve one build per platform, so bundling the staged tree would publish a darwin-arm64-only package. Those are dropped from the payload and their parents' full variant lists are lifted to top-level `optionalDependencies`, letting npm select per install. This covers ripgrep, sharp, koffi, `node-addon-require-builtin`, and the Landlock launcher; `node-pty` and `pi-tui` ship every prebuild inside one package and stay bundled.
...
`pnpm run build:npm-cli` fails the build on an unresolvable runtime edge, which is the property a published tarball cannot recover from. The assembled package was installed from its tarball into a clean project outside the checkout and exercised on the shipped surfaces: `dsh --version`, `dsh --help`, `dsh tui --help`, `dsh --profile headless --help`, and `dsh web --help` all answer, and a bare `dsh` under a pty initializes the `tui` profile, renders the session header, model, permission preset, and key hints, and exits cleanly with its resume hint. The presence of `@deepseek-ai/dsh-tui-app` in the installed tree is what proves the bundled payload was used rather than a registry copy, since that package is not published.
