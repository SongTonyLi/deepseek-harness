...
The reader is two panels and one selection in each. The left panel lists the turns — `▸` on the selected one, its number, the first line of its prompt, and the `⬡ ✻ ¶ ⚒` markers. The right panel is that turn, scrolled row by row, its sections stacked under their own headers. Nothing else is drawn, and nothing is pinned.

One key means one thing per panel. `↑↓` step turns in the list and scroll rows in the turn; `PgUp` / `PgDn` page either; `Home` / `End` reach either end of either; `Right`, `Tab`, and `Enter` cross into the turn; `Left`, `Tab`, and `Shift+Tab` come back to the list; `/` opens the turn filter over the list. `Esc` closes the reader from either panel, so the only Escape chain left inside it is the query line's: clear a non-empty query, then close the line.
