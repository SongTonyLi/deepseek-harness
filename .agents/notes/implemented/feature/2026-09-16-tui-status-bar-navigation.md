...
The footer under the editor is a status bar of segments in a fixed order: the model (`provider/model`), the reasoning effort, the permission preset, the running turn, token usage, `ctx NN%`, `todo done/total`, `goal <phase>`, `plan`, the workspace path (shortened with `~` and `…/` when long), and `<n> attached`. The model, effort, and workspace segments are always present — effort is labelled `effort default` when the selection leaves reasoning effort to the model — and every other segment is present exactly when its fact exists, so an unregistered projection key or an empty attachment list removes its segment rather than showing a placeholder. Unfocused, the bar is one dim line of those key facts (model, effort, a running turn, context percent, todo, workspace) with other present facts folded into a trailing `+N` and a trailing `Shift+↓`; focused, line 1 is a sliding window of navigable segments that always includes the selected one and fits the terminal width, and line 2 expands that segment's first detail row — or, for a page segment, the counts and the item being worked on — plus the navigation hints.

`Shift+Down` moves focus into the bar and selects the first segment: from the editor while no subagent panel is drawn, and from any other region whatever is drawn. While the bar has focus, `Left` / `Right`, `Shift+Left` / `Shift+Right`, and `Tab` / `Shift+Tab` move between segments and wrap at both ends, `Up` leaves the bar for the region drawn above it, `Enter` opens the selected segment while the bar keeps focus, and `Esc` returns focus to the editor. No other key reaches the editor while the bar has focus, so a stray character cannot land in a prompt the user cannot see the caret for; `Ctrl+C` and `Ctrl+D` keep their usual meaning and return focus to the editor, which keeps the quit path reachable from every focus state. `Shift+Left` / `Shift+Right` are not read while the editor has focus, so they stay with text editing. `Shift+Tab` still cycles the bound model's reasoning effort while the editor has focus — the binding is read against the focused component, not globally.

Opening a segment states its current facts and names what changes them: the model segment names `/model`, the effort segment `Shift+Tab`, and the permission segment its preset; the usage, context, goal, and plan segments print the matching sections of the `/status` report; the workspace segment prints the full path the bar shortens; and the attachments segment lists the pending attachments. Those details are printed into the transcript. A segment whose fact the application has its own navigable page for declares that instead of carrying rows, and the bar opens the page: the `todo` segment is that kind, expands the counts and the item being worked on (or the next pending item) on line 2, and opens the same list `/todos` opens. `src/status.ts` owns one set of section builders, and both `/status` and the printed segment details call them, so a section cannot read one way in the report and another way under `Enter`. `src/footer.ts` stays pure: it turns the same facts into the ordered segment list and the selected index plus terminal width into rendered lines, with no Context, terminal, or clock, which is what lets the compact unfocused line, the sliding window, and wrapping be tested without a TUI. `FooterBar` is the mounted component that passes the frame's width into that render.

**Superseded in part.** [Terminal reader overlay, Esc safety, and the editor as the hub](2026-09-18-tui-reader-overlay-and-esc-safety.md) replaces three of the rules above: `Tab` / `Shift+Tab` cycle the regions that are drawn instead of moving the bar's selection, which `Left` / `Right` and the shift arrows already do; the unfocused line is built from both ends instead of ending in a bare `Shift+↓` — the model anchored left and shortened to its bare name past a twenty-column label, both entry keys reserved right, and the key facts filling the middle with the rest folded into `+N`; and a printable key pressed on the bar hands the keyboard back to the editor and types at the caret instead of being consumed there. `Esc` still returns to the editor from the bar, and now starts a window in which an `Esc` reaching the editor does nothing at all. The segment order, what `Enter` opens, and the shared `src/status.ts` sections are unchanged.

## Key bindings over the pi-tui editor

The editor component from `@earendil-works/pi-tui` claims `ctrl+a`, `ctrl+b`, `ctrl+e`, `ctrl+f`, `ctrl+d`, `ctrl+k`, `ctrl+u`, `ctrl+w`, `ctrl+y`, `alt+b`, `alt+d`, `alt+f`, `alt+y`, `tab`, `enter`, `shift+enter`, and the bare arrows for editing, completion, submission, and history. `shift+up` and `shift+down` are the pair it does not claim, so the application reads them without taking a key away from text editing, and both directions of the focus move stay symmetric and memorable. Inside the bar the application owns the whole key stream, which is why `Tab`, `Enter`, the arrows, and `Shift+Left` / `Shift+Right` can mean navigation there while the editor keeps them when it has focus.
...
- `Shift+Tab` is overloaded by focus — effort cycling in the editor, the previous drawn region in the bar. The `/help` keys and the package README state both readings.
