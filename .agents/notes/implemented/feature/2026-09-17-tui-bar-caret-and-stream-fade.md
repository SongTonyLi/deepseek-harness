...
Settled text is never redrawn, in two senses. `AssistantBlock.commit` drops the block's `FadeRender` when the durable `assistant/message` replaces the streamed content, and the application clears the tracker at stream start, stream end, `turn/end`, and session rebinding, so a committed message and a block rebuilt from history carry no tail at all. Within a running stream, a chunk leaves the tail by ageing out and then renders exactly as the Markdown component drew it.
...
While each of the last `FADE_FAST_WINDOW_TICKS` (10) completed ticks received at least one chunk — 400 ms of arrivals at the default period — the stream is delivering text faster than the ramp can show, so every chunk would be replaced before it visibly brightened and the tail would only add a dark band at the stream head. The tracker then stops tracking: the arriving chunk is not recorded and the whole tail is flushed, so nothing already on screen darkens. The decision is recoverable, one tick without an arrival ends it, and recovery affects only chunks appended afterwards, so it never re-darkens settled text. A width change flushes the tail too, because the columns it was matched against no longer describe the rewrapped lines, but keeps the arrival window, since a resize says nothing about the stream's rate.
