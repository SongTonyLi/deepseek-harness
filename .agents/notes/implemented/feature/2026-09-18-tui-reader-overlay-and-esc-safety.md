...
**The reader is a full-screen pane, and it replaces the inline section page.** `Ctrl+G` from any docked state, `Enter` in the transcript, and `/turns` open it — composited into the viewport when this note shipped, and on the terminal's alternate screen since [the terminal reader runs on the alternate screen](../architecture/2026-09-20-tui-reader-on-the-alternate-screen.md): a list of turns with `⬡ ✻ ¶ ⚒` markers, the selected turn scrolling beside it under its own section headers, `/` filtering the list, and `Esc` closing back to the transcript on the section last read — [the reader is two panels](../simplification/2026-09-18-tui-reader-two-panels.md) supersedes the sections walk and the compare pane this note shipped. It holds no copy of the conversation — it re-derives its turn groups from the transcript's own blocks on every render — so a streaming reply grows inside it and a landing tool result appears. `DetailPrompt` stays for the lists `browse` drives — `/todos`, `/subagents`, `/changes`, and the `todo` status-bar segment — and for a subagent panel row, where a bounded page in the modal slot is the right size.
...
## Why a surface composited into the viewport is anchored at its bottom
...
A top-anchored overlay composites at the frame's own viewport top. Whenever the tree frame has shrunk since the renderer's high-water mark — which every `Esc` out of read mode does — that line lies above the boundary and the renderer redraws in full. The reader was anchored `bottom-left` for that reason and told how many of the viewport's lines the renderer could still repaint; it stopped being an overlay at all in [the terminal reader runs on the alternate screen](../architecture/2026-09-20-tui-reader-on-the-alternate-screen.md), which is what `ViewportPad` and the bottom-up reveal existed for. The transient line is placed the same way and remains: it floats within the rows the renderer can still repaint, and falls back to a transcript notice when the viewport's top is out of reach, because a line that says which key stops a turn is worth more than its own disappearance.
...
The reader's own interaction model is superseded in turn by [the reader is two panels](../simplification/2026-09-18-tui-reader-two-panels.md), and where it draws by [the terminal reader runs on the alternate screen](../architecture/2026-09-20-tui-reader-on-the-alternate-screen.md); both keep everything else here current.
