...
这些 flow 之上的界面是[浏览器提供方登录](../feature/2026-09-14-browser-provider-sign-in.zh.md)：`authorization` Remote namespace 把单次尝试的 notice 与 prompt 送到页面，Models 页也新增了发起登录的控件。
...
`models-settings` 与 `onboarding-usable-provider` 两条 web e2e golden 恰好收回了被扣留时失去的那一行 `openai-codex` 选项——这是本决策记录的唯一装配后应用差异。
