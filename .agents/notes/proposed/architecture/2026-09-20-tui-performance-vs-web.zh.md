...
把 TUI 性能当成五个端点。先在生产入口路径上分别测量，再改产品代码。只有不破坏上述终端规则、且有一张具名卡片证明它去掉的就是那笔成本时，才照搬 Web 机制。
...
| 3 | 只在思考间隙停止或放慢加载器 | 降级，直到卡片 D | 一秒思考间隙里的 `doRender` 次数 | 旋转指示器仍表示 Agent 在工作 |
| 4 | 历史画完之前就亮出编辑器 | 降级 | 看不到 transcript 的卡片 A | 会话切换进行中编辑器仍拒绝提交；可打印按键仍回显 |
...
4. 仅当卡片 D 显示加载器帧主导思考间隙 CPU 时做次序 3。
...
在历史画完之前就亮出的首键编辑器，可能对着仍在绑定的会话接受提示词。除非卡片 A 被定为同意的产品改动，否则保持现有「宿主打开另一会话时提交等待」规则。可打印按键在那段等待里已经回显。
