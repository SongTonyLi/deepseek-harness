...
**设置 → 模型**显示 **DeepSeek**（使用 `DEEPSEEK_API_KEY`）以及带「登录」的 **Cursor** 卡片。默认模型为 `deepseek-official` / `deepseek-flash`（DeepSeek-V41-Flash）。[DeepSeek 插件](../../llm/llm-deepseek/README.zh.md#choose-a-protocol)默认使用 Messages；在 Cordis YAML 中设置 `protocol: chat-completions` 可选择 Chat Completions。Web 不提供协议选择器。
