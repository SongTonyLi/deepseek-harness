...
Motion reaches past streamed text, and every effect rides the same fade tick. The keyboard landing on the conversation, the subagent panel, or the status bar lifts that surface's frame, its chip, and the mark it holds for twelve ticks of `streamFadeStepMs`; a step of the conversation walk lifts the newly focused gutter for six; and a step along the bar lifts the selected label for eight. A new or status-changed activity-board todo row, and a replaced descendant line, float out on that tick the same way a tool card does; replay and `reducedMotion` draw the settled board. Running tool cards, folded subagent rows, and the in-progress todo cycle braille spinner frames on their own 80 ms tick, armed only while one of them is drawn; a card whose header has scrolled above the part of the screen the renderer can repaint draws the static `◆` from then on. The reader arrives complete on the screen it owns, and what it newly shows floats out on that tick the way a tool card does: opening it and showing another turn float out the whole turn, and stepping the list to another section of the same turn floats out that section, over `streamFadeSteps` ticks; scrolling, live growth, and a rewrap float nothing out. No lift changes how many lines anything draws, and `reducedMotion` — like `NO_COLOR`, a disabled palette, and `TERM=dumb` — turns every one of them off and arms no repaint for them.
...
`/btw` opens a temporary side agent as a view over the session on screen, so the user can ask questions while that session's agent keeps working. The side agent is a new Agent seeded with every event the session has logged so far, an unfinished turn included: a tool call still waiting for its result gets the fork's error result, and reply text that is still streaming is not included. It runs on the session's last logged model, else its current selection, and it can only read: its session runs in the `read-only` sandbox mode with the `never` approval policy, so a write fails and cannot be approved, and it sees only the tools named in `btwTools`, plus the command tools in `btwSandboxedTools` when the command executor confines commands. Every other tool is hidden from it and refused if called. It runs its own turns: the question after `/btw` is its first prompt, and every line typed while it is on screen goes to it, `/btw <question>` included. The view is drawn like a subagent view: the header ends on `◆ btw side agent · Ctrl+P ends it and returns`, the shaded line above the input reads `◆ btw side agent · main › btw · Ctrl+P back to main`, and the transcript starts empty instead of repeating the inherited conversation. `Ctrl+P` or `/parent` ends the side agent, cancelling a turn it is running, and redraws the session it was opened from under `btw ended · back in session <id>`. The working session is only read: nothing is appended to its log, its inbox is untouched, and neither its subagent panel, `/subagents`, nor its model-facing subagent listing shows the side agent, which also cannot message it. An approval or question that a session held behind any view raises is shown on screen after `session <id>, behind this view, asks:` instead of being refused.
...
| `Up` / `Down` | Recall earlier prompts; while follow-ups wait, `Up` on an empty input removes the newest follow-up into the editor, the same revision as `E` in the follow-ups panel |
...
| `/btw <question>` | Open a temporary side agent with this session's context and ask it the question; it can use read-only tools, the session keeps working and never sees it, and `Ctrl+P` ends it. Inside the side agent, `/btw <question>` asks it another question |
...
| `reducedMotion` | `false` | Draw streamed text, streamed reasoning, tool cards, and the chrome in their settled colors, with no fade, no lift, no spinner glyph or shimmer, and no repeating repaint |
| `openBrowser` | `true` | Hand marked authorization pages to the local default browser |
| `btwTools` | `read`, `read_image`, `glob`, `grep`, `web_search`, `web_fetch`, the `session_*` query tools, `skill`, the MCP resource readers, `ask_user_question` | Global tools a `/btw` side agent may call; names this composition does not register are skipped |
| `btwSandboxedTools` | `bash`, `pwsh` | Command tools a `/btw` side agent may call only when the command executor confines commands; they run in the `read-only` sandbox mode |
...
| [`src/resume-model.ts`](src/resume-model.ts) | Resume-time model selection from the last request header or the launch default |
| [`tests/resume-model.spec.ts`](tests/resume-model.spec.ts) | Resume restores the last logged route, else the launch default |
...
| [`src/btw.ts`](src/btw.ts) | `/btw`: the side agent's seed from the working session's whole log, its read-only tool and sandbox restriction, and its opening notice |
...
| [`src/spinner.ts`](src/spinner.ts) | The braille spinner frames running glyphs cycle through and the shimmer the working spinner's label carries |
...
| [`src/style.ts`](src/style.ts) | The palette, including the cool dark-mode heading and link hues a Markdown reply uses, the strikethrough role completed todo content uses, the background band a prompt is drawn on and the darker one a steered or injected prompt is drawn on, the green and red fills and sign colours of a diff card's changed rows, and the derived pi-tui themes |
...
| [`tests/spinner.spec.ts`](tests/spinner.spec.ts) | Spinner frame steps and the shimmer's sweep, rest, and disabled-palette output |
...
| [`tests/sessions.spec.ts`](tests/sessions.spec.ts) | Session picker listing: newest first, live or cached titles, hidden subagents |
| [`tests/index.spec.ts`](tests/index.spec.ts) | Creation, resume paging, resume model assembly, fork cut, session switching, quit flow, and failure reporting |
...
The working agent's requests are unchanged by `/btw`. The side agent's requests declare only its allowed tools. They start with the working session's inherited history, including the fork's error results for tool calls without a logged result and the closers of an unfinished turn. A `tui-app` notice summarized as `btw · side agent` follows with fixed text: the conversation above belongs to a working agent that is still running; answer the user's questions; its tools are read-only, commands run in a read-only sandbox, and every write is refused and cannot be approved, so it describes a change instead of attempting it; do not continue the working agent's task or message it; and inherited tool calls that end in the fork's error may have finished in the working session. The user's question follows as an ordinary prompt, and later lines are ordinary prompts of the side agent's session.

#### Token effect

Each side agent request carries the inherited history plus the notice and the side agent's own turns. Nothing from the side agent reaches the working agent's requests.

#### KV Cache effect

The side agent uses the working session's last logged provider and model, but its tool catalog is the read-only subset, so a provider that caches the tool declarations ahead of the history recomputes the inherited history on the first side request; later side requests append to that prefix. The working agent's request prefix is unchanged.
