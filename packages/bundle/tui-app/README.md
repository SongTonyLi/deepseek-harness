...
| Key | Effect |
|---|---|
| `Up` / `Down`, `Home` / `End` | Select a pending prompt |
| `S` | Remove the selected prompt from its queue and steer it into the nearest step, waking the Agent |
| `I` | Move the selected prompt to next-step injection without waking the Agent |
| `E` | Remove the selected prompt into the editor; `Enter` sends the revision and `Ctrl+C` restores the original prompt to the next-turn queue |
...
| [`tests/app.spec.ts`](tests/app.spec.ts) | Rendering, keys, commands, the stop arm, the queued-prompt rows above the editor, and both seams over a fake terminal |
