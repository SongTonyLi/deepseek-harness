...
| [`src/style.ts`](src/style.ts) | The palette, including the cool dark-mode heading and link hues a Markdown reply uses, the strikethrough role completed todo content uses, the background band a prompt is drawn on, the green and red fills and sign colours of a diff card's changed rows, and the derived pi-tui themes |
