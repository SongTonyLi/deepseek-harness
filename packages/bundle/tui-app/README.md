...
| Key | Effect |
|---|---|
| `Up` / `Down` | In the turn list, the previous / next turn; in the turn, one row up / down |
| `PageUp` / `PageDown` | A page of turns, or a page of rows with one row of overlap |
| `Home` / `End` | The first / last turn, or the first / last row of the turn |
