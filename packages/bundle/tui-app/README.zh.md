...
| [`src/style.ts`](src/style.ts) | 调色板（含 Markdown 回复所用的暗色冷色阶标题与链接色、已完成 todo 内容所用的划线角色、提示所铺的背景带、diff 卡片变更行的绿色与红色填充及符号颜色）与派生的 pi-tui 主题 |
