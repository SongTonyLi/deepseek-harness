...
| `Up` / `Down` | 调出先前的提示 |
| `Shift+Up` | 从编辑器、面板或状态栏把焦点放到对话记录的最新小节上；对话记录已持有焦点时，走到上一小节 |
| `Shift+Down` | 从编辑器把焦点放到子 agent 面板的第一行（面板已绘制时）或状态栏；从面板跳到状态栏；对话记录已持有焦点时，走到下一小节 |
| `Esc` | 停止正在进行的轮次；已排队的消息保持排队 |
| `Ctrl+O` | 展开或折叠所有工具卡片 |
| `Ctrl+C` | 清空编辑器；600 ms 内再按一次则退出 |
| `Ctrl+D` | 编辑器为空时退出 |

对话记录、子 agent 面板与状态栏按此顺序自上而下排布，`Up` / `Down` 走遍整个序列且在两端都不环绕。对话记录持有焦点时，`Up` / `Down` 与 `Shift+Up` / `Shift+Down` 按阅读顺序走遍每一个小节——系统提示词、注入的上下文（快照的每一份贡献各自作为一部分）、一条消息的推理与回复、一次工具调用的调用与结果，以及你的提示，轮次结束通知与打印出的报告会被跳过——`Left` / `Right` 在块的各部分之间移动且不离开该块，`Enter` 把当前聚焦的部分作为只读页面打开、其中带有该部分的完整各行，离开后回到同一部分，`Esc` 把焦点交还编辑器。各部分承载的是块自身的源文本，因此一条回复读到的是模型写下的 Markdown，而不是其上方绘制出的渲染结果。一条先流出推理再调用工具的消息在可见文本到来之前只暴露推理小节，因此走查不会落在空白回复上。除 `Ctrl+C` 与 `Ctrl+D` 外，其他按键都在此被消费；切换会话会把焦点交回编辑器，尚无可查看内容的会话对 `Shift+Up` 回以 `nothing in the transcript to inspect yet` 并把键盘留在编辑器。

状态栏持有焦点时，`Left` / `Right`、`Shift+Left` / `Shift+Right` 与 `Tab` / `Shift+Tab` 在分段之间移动并在两端环绕，第二行展开所选分段最重要的详情，`Up` 在面板已绘制时离开状态栏前往面板的最后一行、否则前往对话记录，`Enter` 打开所选分段且状态栏保持焦点，`Esc` 把焦点交还编辑器。状态栏持有焦点期间其他按键不会到达编辑器；`Ctrl+C` 与 `Ctrl+D` 保持其一贯含义，并把焦点交还编辑器。
...
| [`src/blocks.ts`](src/blocks.ts) | 对话记录组件：用户提示、assistant 回复、工具卡片、系统提示词与注入上下文、通知；可导航的块还暴露其各小节并绘制焦点标记条 |
...
| [`src/inspector.ts`](src/inspector.ts) | 停靠的检视面板：聚焦小节的标题、带编号且会折行的各部分选择行、各行，及其挂载的组件 |
...
- **历史由终端回滚区持有**——键盘可以走遍对话记录的每个块与每个部分，包括完整画出的系统提示词与注入上下文，但没有搜索；除工具卡片外，其余内容都不折叠；更丰富的导航由浏览器表层持有。
