---
description: "dsh 的交互式终端模式：在你的终端里与 agent（智能体）对话，带流式回复、工具卡片、用键盘浏览对话记录、把各轮次并排读出的全屏阅读器、审批、提问、斜杠命令、@ 引用、附件与会话切换。"
kind: "package-bundle"
---
...
在任一区域按 `Ctrl+G`、在对话记录的某个小节上按 `Enter`，以及 `/turns`，都会整屏读出对话记录。阅读器是合成进视口的浮层，因此不占用对话记录的任何一行，也让重绘边界保持原位。其顶边线带有 ` ● READER ` 徽标、`turn <n> of <total>` 与对话记录中的位置；左侧的轮次栏每个轮次一行——被持有的那行带 `▸`、轮次编号、其提示的首行，以及右对齐的标记 `⬡<n>` 注入上下文块数、`✻` 有推理、`¶` 有回复、`⚒<n>` 工具调用数——因此各轮次一眼可比；旁边的窗格把被持有轮次的各小节堆叠在 `── <label> · turn <n> ` 标题之下，被持有的小节加粗，且其每一行都带强调色的 `▌`。那里的行从不折叠：阅读器正是读取系统提示词、注入快照、长回复或工具结果完整面向模型文本的地方。在各小节中按 `Enter` 把被持有的小节固定进第二个窗格，于是来自不同提示的两个小节并排出现，在已固定的小节上按 `Enter` 取消固定；列数不足 `readerMinColumns` 时固定仍被记录，但只绘制一个窗格，按键提示改为 `compare needs <n> columns`。`/` 在按键提示的位置打开一行查询，把轮次栏收窄到提示或文本匹配的轮次，读数写作 `<kept>/<total> turns`；其余时候提示边线在右端报告 `turn 3/12 · section 2/9 · row 14/212`，任何宽度都不会丢弃它。不足 60 列时正文一次只画一栏，不足 24 列或 8 行时只画一行暗色文字 `terminal too small for the reader (needs 24×8)`，并且只应答 `Esc`、`Ctrl+G` 与 `Ctrl+C`。它在每一帧都重新读取对话记录，因此流式回复会在其中增长、落地的工具结果会出现、新轮次会加入轮次栏；改变终端大小时它按正在阅读的小节重新锚定，而不是按行号；审批或提问出现时它在该提示存续期间让位；切换会话会让它随通知 `the transcript changed · reader closed` 一同关闭。
...
| 按键 | 效果 |
|---|---|
| `Up` / `Down` | 在轮次栏中是上一个 / 下一个轮次；在各小节中是上一个 / 下一个小节，并滚动到可见处 |
| `Shift+Up` / `Shift+Down` | 在各小节中滚动一行，选择保持不变 |
| `PageUp` / `PageDown` | 翻一页轮次，或翻一页行并重叠一行 |
| `Home` / `End` | 第一个 / 最后一个轮次，或被持有轮次的第一个 / 最后一个小节 |
| `Right` | 从轮次栏进入被持有轮次的各小节；在各小节中是下一个小节 |
| `Left` | 回到轮次栏 |
| `Tab` / `Shift+Tab` | 在轮次栏中 `Tab` 打开其各小节；在各小节中两者都回到轮次栏 |
| `1`…`9` | 在各小节中，选中被持有轮次里编号对应的小节 |
| `Enter` | 在轮次栏中打开该轮次的各小节；在各小节中把被持有的小节固定到走查旁边，或取消已固定的那个 |
| `/` | 在轮次栏中打开轮次过滤：可打印键扩展查询，`Backspace` 删除一个字符，`Ctrl+U` 清空，`Enter` 保留收窄后的轮次栏并关闭该行，`Esc` 先清空非空查询、再关闭该行 |
| `Esc` | 取消固定的小节；窄终端上从各小节回到轮次栏；其余情况关闭阅读器，并回到对话记录上最后阅读的那个小节 |
...
| `readerMinColumns` | `80` | 阅读器并排绘制两个小节所需的列数；不足时固定仍被记录，其按键提示写作 `compare needs <n> columns` |
