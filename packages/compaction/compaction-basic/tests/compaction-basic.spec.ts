...
describe('default one-shot summarizer', () => {
  ...
  it('pins work-state checkpoint sections after Next Step and inspection-complete Current Work', async () => {
    const { adapter, compact } = await summarizerHarness([{ type: 'text', text: 'summary' }])
    ...
    const lastText = last?.type === 'text' ? last.text : ''
    expect(lastText).toContain('do not tell the successor to re-read unless the file changed')
    expect(lastText).toContain('what was chosen and why it is binding')
    expect(lastText).toContain('must NOT reopen "inspect owners"')
    expect(lastText).toContain('still-true decisions')
    expect(lastText).toContain('quote error text verbatim')
    const headings = [...lastText.matchAll(/^## .+$/gm)].map(match => match[0])
...
describe('model-switch work-state notice', () => {
  const NOTICE_CHECKPOINT_NO_EDITS = '[work state: treat the latest <compacted-summary> and durable session/workspace state as authority; do not re-inspect completed work or re-derive a plan already recorded; the next action is the checkpoint\'s Next Step, or a state-changing tool (read only if a named file is missing); the previous turn made no file changes; do not repeat its plan]'
  const NOTICE_CHECKPOINT_EDITS = '[work state: treat the latest <compacted-summary> and durable session/workspace state as authority; do not re-inspect completed work or re-derive a plan already recorded; the next action is the checkpoint\'s Next Step, or a state-changing tool (read only if a named file is missing)]'
  const NOTICE_ACT_NO_EDITS = '[work state: do not re-inspect completed work or re-derive a plan already recorded; the next action is a state-changing tool (read only if a named file is missing); the previous turn made no file changes; do not repeat its plan]'
  const NOTICE_ACT_EDITS = '[work state: do not re-inspect completed work or re-derive a plan already recorded; the next action is a state-changing tool (read only if a named file is missing)]'

  function modelSelectionNotice(
    from = 'deepseek-official/a',
    to = 'cursor/composer-2',
  ) {
  ...
  function workStateSource() {
    return { kind: 'plugin' as const, plugin: 'compaction-basic', form: 'notice' as const, summary: 'work state' }
  }

  function admit(
    ctx: Context,
    owner: Agent,
    next: () => Promise<PreStepDecision> = () => Promise.resolve({
      kind: 'enter',
      messages: [modelSelectionNotice()],
    }),
  ) {
    return agentEvents(ctx, owner).waterfall(
      'agent/pre-step',
  ...
  function workStateEngine(session: Session) {
    const ctx = createContext()
    void new BasicCompactionEngine(ctx, { auto: false })
    return { ctx, owner: agent(session, MODEL) }
  }

  function mutatingTurn(name: string, isError = false): Session {
    const session = Session.create(SessionId(`work-state-${name}-${String(isError)}`))
    const callId = ToolCallId(`call-${name}`)
    session.append('turn/start', { turn: 1 })
    session.append('user/message', createUserMessage({
  ...
  function withCheckpoint(session: Session): Session {
    session.append('user/message', createUserMessage({
      content: frameSummary([{ type: 'text', text: '## Next Step\n- apply the remaining edit' }]),
      source: { kind: 'plugin', plugin: 'test' },
    }), { surfaceOp: 'append' })
    return session
  }

  function noticeText(decision: PreStepDecision): string {
    if (decision.kind !== 'enter') return ''
    const last = decision.messages.at(-1)?.content[0]
    return last?.type === 'text' ? last.text : ''
  }

  it('does not inject without a model-selection notice and still calls next()', async () => {
    const { ctx, owner } = workStateEngine(conversation(1))
    const next = vi.fn(() => Promise.resolve({ kind: 'enter' as const, messages: [] }))
    await expect(admit(ctx, owner, next)).resolves.toEqual({ kind: 'enter', messages: [] })
    expect(next).toHaveBeenCalledOnce()
  })

  it('returns a reject decision unchanged after next()', async () => {
    const { ctx, owner } = workStateEngine(conversation(1))
    const next = vi.fn(() => Promise.resolve({ kind: 'reject' as const }))
    await expect(admit(ctx, owner, next)).resolves.toEqual({ kind: 'reject' })
    expect(next).toHaveBeenCalledOnce()
  })

  it('injects the shorter act-do-not-replan notice when history has no checkpoint', async () => {
    const { ctx, owner } = workStateEngine(conversation(1))
  ...
  it('treats a session with no completed turn as having no file changes', async () => {
    const { ctx, owner } = workStateEngine(Session.create(SessionId('empty-work-state')))
    expect(noticeText(await admit(ctx, owner))).toBe(NOTICE_ACT_NO_EDITS)
  })

  it('treats the latest compacted-summary as authority when one is already in history', async () => {
    const { ctx, owner } = workStateEngine(withCheckpoint(conversation(1)))
    const decision = await admit(ctx, owner)
    expect(noticeText(decision)).toBe(NOTICE_CHECKPOINT_NO_EDITS)
  })

  it.each(['edit', 'write', 'bash'] as const)(
    'omits the no-file-changes clause after a successful %s call',
    async (name) => {
      const { ctx, owner } = workStateEngine(mutatingTurn(name))
      expect(noticeText(await admit(ctx, owner))).toBe(NOTICE_ACT_EDITS)
    },
  )

  it('keeps the no-file-changes clause after a failed bash or a successful read', async () => {
    const failed = workStateEngine(mutatingTurn('bash', true))
    expect(noticeText(await admit(failed.ctx, failed.owner))).toBe(NOTICE_ACT_NO_EDITS)
    const readOnly = workStateEngine(mutatingTurn('read'))
    expect(noticeText(await admit(readOnly.ctx, readOnly.owner))).toBe(NOTICE_ACT_NO_EDITS)
  })

  it('uses the checkpoint notice without the no-file-changes clause after a successful write', async () => {
    const { ctx, owner } = workStateEngine(withCheckpoint(mutatingTurn('write')))
    expect(noticeText(await admit(ctx, owner))).toBe(NOTICE_CHECKPOINT_EDITS)
  })

  it('does not append a second work-state notice when one is already admitted', async () => {
    const { ctx, owner } = workStateEngine(conversation(1))
    const existing = createUserMessage({
      content: [{ type: 'text', text: NOTICE_ACT_NO_EDITS }],
      source: workStateSource(),
    })
    const decision = await admit(ctx, owner, () => Promise.resolve({
      kind: 'enter',
      messages: [modelSelectionNotice(), existing],
    }))
    expect(decision.kind === 'enter' ? decision.messages : []).toHaveLength(2)
  })

  it('appends after a later prepended model-selection listener once the agent is created', async () => {
    const ctx = createContext()
    void new BasicCompactionEngine(ctx, { auto: false })
    ...
    await ctx.serial('agent/created', { agent: owner, source: 'startup' })
  ...
  it('injects when a same-provider Cursor switch keeps a bare incoming model id', async () => {
    const session = conversation(1)
    session.append('request/header', {
      header: { config: { provider: 'cursor', model: 'composer-1' } },
      reason: 'change',
    })
    const { ctx, owner } = workStateEngine(session)
    const decision = await admit(ctx, owner, () => Promise.resolve({
      kind: 'enter',
      messages: [modelSelectionNotice('composer-1', 'composer-2')],
    }))
    expect(noticeText(decision)).toBe(NOTICE_ACT_NO_EDITS)
  })

  it('does not inject when the incoming route is a pi-ai catalog provider', async () => {
    const { ctx, owner } = workStateEngine(conversation(1))
    const decision = await admit(ctx, owner, () => Promise.resolve({
      kind: 'enter',
      messages: [modelSelectionNotice('deepseek-official/a', 'openai-codex/gpt-5')],
    }))
    expect(decision.kind === 'enter' ? decision.messages : []).toHaveLength(1)
  })

  it('does not inject when the session continues onto a DeepSeek route', async () => {
    const session = conversation(1)
    session.append('request/header', {
    ...
    const decision = await admit(ctx, owner, () => Promise.resolve({
      kind: 'enter',
      messages: [modelSelectionNotice('cursor/composer-2', 'deepseek-official/deepseek-chat')],
    }))
    expect(decision.kind === 'enter' ? decision.messages : []).toHaveLength(1)
  })

  it('does not inject a same-provider switch whose previous header is not cursor', async () => {
    const { ctx, owner } = workStateEngine(conversation(1))
    const decision = await admit(ctx, owner, () => Promise.resolve({
      kind: 'enter',
      messages: [modelSelectionNotice('a', 'b')],
    }))
    expect(decision.kind === 'enter' ? decision.messages : []).toHaveLength(1)
  })

  it('does not inject a bare incoming model id when the session has no request header', async () => {
    const { ctx, owner } = workStateEngine(Session.create(SessionId('no-header-work-state')))
    const decision = await admit(ctx, owner, () => Promise.resolve({
      kind: 'enter',
      messages: [modelSelectionNotice('a', 'composer-2')],
    }))
    expect(decision.kind === 'enter' ? decision.messages : []).toHaveLength(1)
  })

  it('does not inject when the model-selection notice has no incoming route label', async () => {
    const { ctx, owner } = workStateEngine(conversation(1))
    const notice = createUserMessage({
      content: [
        { type: 'image', attachment: { attachmentId: 'img' } as never },
        { type: 'text', text: '[model changed]' },
      ],
      source: { kind: 'plugin', plugin: 'model-selection', form: 'notice', summary: 'changed' },
    })
    const decision = await admit(ctx, owner, () => Promise.resolve({
      kind: 'enter',
      messages: [
