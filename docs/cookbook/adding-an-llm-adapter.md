...
How to connect a new model provider. Reference implementations: `packages/llm/llm-deepseek` (direct HTTP, SSE framed by `eventsource-parser`), `packages/llm/llm-pi-ai` (wrapping an LLM library), and `packages/llm/llm-cursor` (unofficial Connect/protobuf HTTP/2). Read the `StreamChunk` doc in `packages/llm/llm/src/types.ts` first — it records the protocol conventions the adapters were verified against.
