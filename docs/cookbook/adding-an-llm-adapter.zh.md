...
如何接入一个新的模型提供方。参考实现：`packages/llm/llm-deepseek`（直接 HTTP，SSE（Server-Sent Events）由 `eventsource-parser` 分帧）、`packages/llm/llm-pi-ai`（封装 LLM 库）与 `packages/llm/llm-cursor`（非官方 Connect/protobuf HTTP/2）。请先阅读 `packages/llm/llm/src/types.ts` 中的 `StreamChunk` 文档——它记录了适配器都经过验证的协议约定。
